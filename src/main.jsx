...
function Login({ onSession }) { const [mode, setMode] = useState('login'); const [notice, setNotice] = useState(''); const [noticeIsError, setNoticeIsError] = useState(false); const [submitting, setSubmitting] = useState(false); const navigate = useNavigate(); const submit = async (e) => { e.preventDefault(); const form = new FormData(e.currentTarget); if (!supabase) { setNoticeIsError(true); return setNotice('Supabase browser credentials are required. Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to .env.'); } setSubmitting(true); const values = Object.fromEntries(form); const emailRedirectTo = import.meta.env.VITE_AUTH_REDIRECT_URL || `${window.location.origin}/login`; const { data, error } = mode === 'login' ? await supabase.auth.signInWithPassword({ email: values.email, password: values.password }) : await supabase.auth.signUp({ email: values.email, password: values.password, options: { emailRedirectTo, data: { full_name: values.name } } }); if (error) { setSubmitting(false); setNoticeIsError(true); return setNotice(error.message); } if (data.session) { onSession(data.session); navigate('/portal', { replace: true }); return; } setSubmitting(false); setNoticeIsError(false); setNotice('Check your email to verify your new account.'); }; const switchMode = () => { if (submitting) return; setNotice(''); setMode(mode === 'login' ? 'register' : 'login'); }; return <main className="auth-shell"><form key={mode} className={`card-form auth-form auth-form-${mode}`} onSubmit={submit}><Link className="brand" to="/"><span>✦</span> bright<span>smile</span></Link><h1>{mode === 'login' ? 'Welcome back.' : 'Create your account.'}</h1>{mode === 'register' && <label>Full name<input required name="name" placeholder="Your name" /></label>}<label>Email<input required type="email" name="email" placeholder="you@email.com" /></label><label>Password<input required type="password" name="password" minLength="8" placeholder="At least 8 characters" /></label>{notice && <p className={noticeIsError ? 'form-error' : 'success'}>{notice}</p>}<button disabled={submitting} className={`button${submitting ? ' is-loading' : ''}`}>{submitting ? (mode === 'login' ? 'Logging in…' : 'Creating account…') : (mode === 'login' ? 'Log in →' : 'Create account →')}</button><button type="button" className="text-button" disabled={submitting} onClick={switchMode}>{mode === 'login' ? 'New here? Create an account' : 'Already have an account? Log in'}</button></form></main> }
...
function AdminDashboard({ session }) {
  ...
  const createService = async (event) => {
    ...
    try { await api('/admin/services', { token, method: 'POST', body: JSON.stringify({ name: form.name, description: form.description, price: Number(form.price), durationMinutes: Number(form.durationMinutes), icon: form.icon }) }); event.currentTarget.reset(); setNotice('Service created.'); await load(); } catch (e) { setError(e.message); }
  };
  const createDentist = async (event) => {
    event.preventDefault(); const form = Object.fromEntries(new FormData(event.currentTarget));
    try { await api('/admin/dentists', { token, method: 'POST', body: JSON.stringify({ fullName: form.fullName, email: form.email, password: form.password, specialty: form.specialty, licenseNumber: form.licenseNumber, phone: form.phone, appointmentDurationMinutes: Number(form.appointmentDurationMinutes) }) }); event.currentTarget.reset(); setNotice('Dentist account created.'); await load(); } catch (e) { setError(e.message); }
  };

  const title = adminSections.find(([id]) => id === section)?.[1] || 'Dashboard';
  return <main className="super-admin">
    <aside className="admin-sidebar"><Link className="brand" to="/"><span>✦</span> bright<span>smile</span></Link><p>Super Admin</p><nav aria-label="Admin navigation">{adminSections.map(([id, label]) => <button key={id} className={section === id ? 'active' : ''} onClick={() => { setSection(id); setQuery(''); setStatusFilter('all'); }}>{label}</button>)}</nav></aside>
    <section className="admin-workspace">
      <header className="admin-topbar"><div><div className="eyebrow">Clinic command center</div><h1>{title}</h1></div><button className="button button-small" onClick={() => { setSection('bookings'); setBookingFormOpen(true); }}>Create booking</button></header>
      ...
      {loading ? <LoadingState compact label="Loading live clinic data…"/> : <>
        {section === 'dashboard' && <AdminOverview overview={overview} onNavigate={setSection}/>} 
        ...
        {section === 'patients' && <RecordList title="Patient records" rows={visibleRows} query={query} setQuery={setQuery} kind="patient"/>}
        {section === 'accounts' && <AccountManager rows={visibleRows} query={query} setQuery={setQuery} update={update}/>} 
        ...
        {section === 'messages' && <RecordList title="Customer messages" rows={visibleRows} query={query} setQuery={setQuery} kind="message"/>}
...
function AdminOverview({ overview, onNavigate }) { const counts = overview?.counts || {}; return <><div className="admin-metric-grid">{[{ label: 'Patients', value: counts.totalPatients }, { label: 'Accounts', value: counts.totalAccounts }, { label: 'Dentists', value: counts.totalDentists }, { label: 'Today', value: counts.todayAppointments }, { label: 'Pending', value: counts.pending }, { label: 'Confirmed', value: counts.confirmed }, { label: 'Completed', value: counts.completed }, { label: 'Cancelled', value: counts.cancelled }, { label: 'No-show', value: counts.noShow }].map((item) => <article key={item.label}><span>{item.label}</span><b>{item.value ?? 0}</b></article>)}</div><div className="admin-quick-actions"><button onClick={() => onNavigate('patients')}>Add patient</button><button onClick={() => onNavigate('dentists')}>Add dentist</button><button onClick={() => onNavigate('services')}>Add service</button><button onClick={() => onNavigate('accounts')}>Manage accounts</button><button onClick={() => onNavigate('bookings')}>View all bookings</button></div><div className="admin-split"><section className="admin-card"><h2>Recent bookings</h2>{overview?.recentBookings?.length ? overview.recentBookings.map((booking) => <article key={booking.id}><b>{booking.patient_name}</b><span>{booking.services?.name} · {booking.dentist?.display_name || 'Dentist'}</span><time>{new Date(booking.starts_at).toLocaleString()}</time></article>) : <p>No recent bookings.</p>}</section><section className="admin-card"><h2>Recent activity</h2>{overview?.recentActivity?.length ? overview.recentActivity.map((entry) => <article key={entry.id}><b>{entry.action}</b><span>{entry.entity_type}</span><time>{new Date(entry.created_at).toLocaleString()}</time></article>) : <p>No administrative activity yet.</p>}</section></div></> }
...
function AdminBookingForm({ token, onCreated, onError }) {
  ...
  const submit = async (event) => {
    ...
    if (Number.isNaN(startsAt.getTime())) return setMessage('Choose a valid appointment date and time.');
...
function AccountManager({ rows, query, setQuery, update }) { return <section className="admin-card"><h2>Accounts</h2><SearchBar query={query} setQuery={setQuery} placeholder="Name, email, phone, ID, or role"/><div className="responsive-table account-table"><div className="table-head"><span>Account</span><span>Role</span><span>Status</span></div>{rows.map((item) => <article key={item.id}><div><b>{item.full_name || 'Unnamed account'}</b><span>{item.email}</span><small>{item.id}</small></div><select value={item.role} onChange={(event) => update(`/admin/accounts/${item.id}`, { role: event.target.value }, 'Role updated.')}>{['patient', 'dentist', 'staff', 'admin'].map((role) => <option key={role} value={role}>{formatStatus(role)}</option>)}</select><button className={item.is_active ? 'table-action danger' : 'table-action'} onClick={() => update(`/admin/accounts/${item.id}`, { isActive: !item.is_active }, item.is_active ? 'Account deactivated.' : 'Account reactivated.')}>{item.is_active ? 'Deactivate' : 'Reactivate'}</button></article>)}</div></section> }
