import React, { StrictMode, Suspense, lazy, useEffect, useMemo, useRef, useState } from 'react';
...
function Header({ session, profile, onRequestSignOut, signingOut }) {
  ...
  return <header className="site-header"><Link className="brand brand-logo" to="/"><img src="/bright-smile-mark.svg" alt=""/>bright<span>smile</span></Link><button className="menu-button" aria-label={open ? 'Close menu' : 'Open menu'} aria-expanded={open} aria-controls="site-navigation" onClick={() => setOpen(!open)}>{open ? '×' : '☰'}</button><nav id="site-navigation" className={open ? 'open' : ''} onClick={() => setOpen(false)}>
    ...
    {session ? <><NavLink to="/portal">My visits</NavLink>{profile?.role === 'admin' && <NavLink to="/admin">Admin</NavLink>}<button className={`text-button signout-button${signingOut ? ' is-signing-out' : ''}`} onClick={onRequestSignOut} disabled={signingOut}>{signingOut ? 'Signing out…' : 'Sign out'}</button></> : <NavLink to="/login">Log in</NavLink>}
    <Link className="button button-small" to="/book">Book a visit</Link>
  </nav></header>;
}

function SignOutDialog({ open, busy, onCancel, onConfirm }) {
  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onCancel(); }}>
    <section className="confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="signout-title" aria-describedby="signout-description">
      <div className="dialog-mark" aria-hidden="true">↗</div>
      <h2 id="signout-title">Sign out?</h2>
      <p id="signout-description">You can sign back in whenever you need to manage appointments or messages.</p>
      <div className="dialog-actions"><button ref={cancelButton} type="button" className="table-action" onClick={onCancel} disabled={busy}>Stay signed in</button><button type="button" className={`button button-danger${busy ? ' is-loading' : ''}`} onClick={onConfirm} disabled={busy}>{busy ? 'Signing out…' : 'Yes, sign out'}</button></div>
    </section>
  </div>;
}
...
function App() { const [session, setSession] = useState(null); const [profile, setProfile] = useState(null); const [authLoading, setAuthLoading] = useState(true); const [profileLoading, setProfileLoading] = useState(false); const [signingOut, setSigningOut] = useState(false); const [signOutDialogOpen, setSignOutDialogOpen] = useState(false); useEffect(() => { if (!supabase) { setAuthLoading(false); return; } supabase.auth.getSession().then(({ data }) => setSession(data.session)).finally(() => setAuthLoading(false)); const { data: listener } = supabase.auth.onAuthStateChange((_event, next) => { setSession(next); setAuthLoading(false); }); return () => listener.subscription.unsubscribe(); }, []); useEffect(() => { if (!session) { setProfile(null); setProfileLoading(false); return; } setProfileLoading(true); api('/auth/me', { token: session.access_token }).then(setProfile).catch(() => setProfile(null)).finally(() => setProfileLoading(false)); }, [session]); const signOut = async () => { if (signingOut) return; setSigningOut(true); try { await supabase?.auth.signOut(); setSession(null); } finally { setSigningOut(false); setSignOutDialogOpen(false); } }; const loadingRoute = <main className="route-loading"><LoadingState label="Loading your account…"/></main>; const profileRouteLoading = <main className="route-loading"><LoadingState label="Loading your dashboard…"/></main>; const adminRoute = authLoading ? loadingRoute : !session ? <Navigate to="/login" replace/> : profileLoading ? profileRouteLoading : profile?.role === 'admin' ? <AdminDashboard session={session}/> : <Navigate to="/" replace/>; const portalRoute = authLoading ? loadingRoute : !session ? <Navigate to="/login" replace/> : profileLoading ? profileRouteLoading : profile?.role === 'admin' ? <Navigate to="/admin" replace/> : <Portal session={session}/>; const signedInDestination = profile?.role === 'admin' ? '/admin' : '/portal'; return <><Header session={session} profile={profile} onRequestSignOut={() => setSignOutDialogOpen(true)} signingOut={signingOut}/><Routes><Route path="/" element={<Home/>}/><Route path="/services" element={<Services/>}/><Route path="/about" element={<About/>}/><Route path="/contact" element={<Contact/>}/><Route path="/book" element={<Book session={session}/>}/><Route path="/login" element={authLoading ? loadingRoute : session ? profileLoading ? profileRouteLoading : <Navigate to={signedInDestination} replace/> : <Login onSession={setSession}/>}/><Route path="/portal" element={portalRoute}/><Route path="/admin" element={adminRoute}/><Route path="*" element={<Navigate to="/" replace/>}/></Routes><footer><span className="brand"><span>✦</span> bright<span>smile</span></span><p>Thoughtful dentistry for brighter days.</p><p>© 2026 Bright Smile Dental Clinic</p></footer><SignOutDialog open={signOutDialogOpen} busy={signingOut} onCancel={() => setSignOutDialogOpen(false)} onConfirm={signOut}/></> }
